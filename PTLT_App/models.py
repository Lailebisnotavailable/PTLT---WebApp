...
class FingerprintRegistration(models.Model):
    ...
    fingerprint_data = models.BinaryField(verbose_name="Fingerprint Data")  # stores binary template
    date_registered = models.DateTimeField(default=timezone.now, verbose_name="Date of Registration")

    device_id = models.CharField(max_length=100, verbose_name="Registered From Device", blank=True, null=True)  # optional, for tracking source tablet

    synced = models.BooleanField(default=False, verbose_name="Synced to Cloud")  # to track upload status
...
class ClassSchedule(models.Model):
    ...
    def __str__(self):
        return f"{self.course_code} - {self.section} ({self.professor.last_name})"
...
class AttendanceRecord(models.Model):
    ...
    def __str__(self):
        return f"{self.date} - {self.student.account.user_id} - {self.status}"
